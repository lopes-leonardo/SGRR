...
cdef class LhrrWrapper:
    ...
    def run(self,
        ...
        if x == y:
            ...
            distances = metrics.pairwise.cosine_distances(np_data, np_data)
            self._init_with_distance_matrix(distances)
        else:
    ...
    def get_similarity_matrix(self):
        vector_sm = np.asarray(self.lhrr.getSimilarityMatrix())
        similarity_matrix = []
        for i in range(self.n):
            start = self.n * i
            end = start + self.n
            similarity_matrix.append(vector_sm[start:end])
